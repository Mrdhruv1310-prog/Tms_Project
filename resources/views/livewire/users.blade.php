<div
    class="min-h-screen bg-gradient-to-br from-slate-100 via-indigo-50/50 to-blue-100/60 antialiased selection:bg-blue-600 selection:text-white text-slate-800">
    <main
        class="scrollcontainer px-3 sm:px-6 md:px-8 pb-20 pt-20 sm:pt-24 md:ml-16 max-w-[1750px] mx-auto transition-all duration-300">
        ...
        <div x-data="{ userDeleteModalOpen: false, userId: null }" @userdeleted.window="userDeleteModalOpen = false"
            class="overflow-hidden rounded-2xl sm:rounded-3xl border border-blue-200/80 bg-white/95 backdrop-blur-xl shadow-2xl shadow-blue-500/10 ring-2 ring-white">
            <div
                class="border-b border-slate-200 px-6 sm:px-8 py-5 sm:py-6 bg-gradient-to-r from-slate-50 via-blue-50/60 to-indigo-50/30 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex flex-col gap-1">
                    <div class="flex items-center gap-2.5">
                        <span
                            class="h-3 w-3 rounded-full bg-blue-600 animate-pulse ring-4 ring-blue-200 shadow-md shadow-blue-500"></span>
                ...
                <div
                    class="text-xs font-semibold text-slate-800 bg-white px-4 py-2 rounded-xl border border-blue-200 shadow-sm self-start sm:self-auto flex items-center gap-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 sm:gap-6">
                    @foreach ($users as $user)
                        ...
                        <div wire:key="{{ $user->id }}"
                            ...
                            <div
                                class="absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r from-blue-600 via-indigo-600 to-sky-500 rounded-t-2xl opacity-75 group-hover:opacity-100 transition-opacity duration-300 shadow-sm">
                            </div>
                            ...
                            <span
                                class="mb-2.5 inline-flex rounded-full bg-blue-50 px-3.5 py-1 text-[10px] sm:text-xs font-bold uppercase tracking-wider text-blue-700 border border-blue-200 shadow-xs">
                                {{ Str::ucfirst($user->role) }}
                            </span>

                            <h3
                                class="truncate w-full text-sm sm:text-base font-bold text-slate-900 mb-4 px-2 tracking-tight">
                                {{ Str::ucfirst($user->first_name) . ' ' . Str::ucfirst($user->last_name) }}
                            </h3>

                            <div
                                class="flex items-center justify-center gap-2.5 w-full mt-auto pt-3 border-t border-slate-100">
                                <button wire:click="$dispatch('edituser', { id: {{ $user->id }} })"
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg"
                                        class="relative z-10 w-4 h-4 transition-transform duration-300 group-hover/btn:rotate-12"
                                ...
                                @if (auth()->user()->id !== $user->id)
                                    <button @click="userDeleteModalOpen=true;userId={{ $user->id }}"
                                        ...
                                        <svg class="relative z-10 w-4 h-4 transition-transform duration-300 group-hover/btn:rotate-12"
                        ...
                        <div class="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0">
                            <div
                                ...
                                <div class="sm:flex sm:items-start gap-4">
                                    <div
                                        class="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl bg-rose-100 text-rose-600 sm:mx-0 sm:h-12 sm:w-12 shadow-inner ring-4 ring-rose-50">
                                    ...
                                    <div class="mt-3 text-center sm:ml-2 sm:mt-0 sm:text-left">
                                        ...
                                        <div class="mt-1">
                                            <p class="text-xs sm:text-sm font-medium text-slate-600">Are you sure you
