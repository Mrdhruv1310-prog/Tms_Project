            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4">
                    @foreach ($groups as $group)
                        @php
                            $circumference = 565.48;
                            $offset = $circumference - ($circumference * $group['percentage']) / 100;
                            // Determine dot color based on completion percentage
                            if ($group['percentage'] >= 71) {
                                $dotColor = 'bg-green-500';
                            } elseif ($group['percentage'] >= 31) {
                                $dotColor = 'bg-yellow-300';
                            } else {
                                $dotColor = 'bg-red-600';
                            }
                        @endphp
                        <a href="{{ route('group.details', ['id' => $group['id']]) }}" wire:navigate class="block">
                            <div class="relative bg-white px-4 py-2 rounded-lg shadow-md">
                                <!-- Dot for the status -->
                                <div class="absolute top-2 right-2 w-4 h-4 rounded-full {{ $dotColor }}"></div>
                                ...
                                <div class="flex items-center">
                                    <!-- SVG Progress Bar for Completed Tasks -->
                                    <div class="w-20 h-20">
                                        <svg width="100%" height="100%" viewBox="-25 -25 250 250"
                                            xmlns="http://www.w3.org/2000/svg" style="transform: rotate(-90deg)">
                                    ...
                                    <div class="ml-4">
                                        <p class="text-sm font-medium text-gray-600">
                                            Pending: {{ $group['pending'] }}/{{ $group['total'] }}
                                        </p>
                                        <p class="text-sm font-medium text-gray-600">
                                            In Progress: {{ $group['in_progress'] }}/{{ $group['total'] }}
                                        </p>
                                        <p class="text-sm font-medium text-gray-600">
                                            Completed: {{ $group['completed'] }}/{{ $group['total'] }}
                                        </p>
                                    </div>
                                </div>
                            </div>
