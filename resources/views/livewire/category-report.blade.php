            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
                @foreach ($categories as $category)
                    <div class="group bg-white px-4 py-2 rounded-lg shadow-md hover:bg-blue-600 hover:text-white">
                        ...
                        <!-- Progress bars and statuses for Pending, In Progress, Completed -->
                        <div class="space-y-4 mt-4">
                            @foreach (['pending' => '#f87171', 'in_progress' => '#fbbf24', 'completed' => '#67d697'] as $status => $color)
                                @php
                                    ...
                                    // Calculate stroke offset for the progress bar
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="w-11 h-11">
                                        <svg width="100%" height="100%" viewBox="-25 -25 250 250" xmlns="http://www.w3.org/2000/svg" style="transform: rotate(-90deg)">
                                            ...
                                            <text class="fill-current text-black group-hover:text-white" x="45px" y="110px" fill="#000000" font-size="50px" font-weight="bold" style="transform: rotate(90deg) translate(0px, -196px)">
                                                {{ (int)$percentage }}%
                                            </text>
                                        </svg>
                                    </div>
                                    ...
                                    <div class="ml-4">
                                        <p class="text-sm font-medium text-gray-600 group-hover:text-white">
                                            {{ ucwords($status) }} - {{ $completed }}/{{ $total }}
                                        </p>
                                    </div>
                                </div>
                            @endforeach
