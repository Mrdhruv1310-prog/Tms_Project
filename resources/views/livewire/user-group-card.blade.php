<div class="antialiased dark:bg-gray-900 min-h-screen">
    ...
    <main class="p-4 sm:p-6 md:p-8 md:ml-16 h-auto pt-20 sm:pt-24 md:pt-20 pb-16 max-w-7xl mx-auto">
        ...
        <!-- Top Banner / Header Container - Clean and Subtly Highlighted -->
        <div class="bg-gradient-to-r from-blue-50/60 via-white to-slate-50/80 dark:from-gray-800 dark:via-gray-800 dark:to-blue-950/30 rounded-2xl shadow-sm border border-blue-200/60 dark:border-blue-800/40 p-6 sm:p-8 mb-6">
            <div class="flex items-center space-x-2 text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">
                <button type="button" class="hover:text-blue-600 dark:hover:text-blue-400 transition-colors inline-flex items-center gap-1" @click="window.history.back()">
                    <span>&lt; Back</span>
                </button>
                <span class="text-gray-400">›</span>
                <span class="text-blue-600 dark:text-blue-400 uppercase tracking-wider text-[11px] font-bold">USER GROUPS MANAGEMENT</span>
            </div>
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 class="text-xl sm:text-2xl md:text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight mb-1">
                        Manage User Groups
                    </h1>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200/80 dark:border-gray-700 p-6 sm:p-8 mb-6">
            <div class="mb-4">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200/80 dark:border-gray-700 p-6 sm:p-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                ...
                <div class="inline-flex items-center px-3.5 py-1.5 bg-blue-50/80 dark:bg-blue-900/30 border border-blue-200/60 dark:border-blue-700/50 rounded-full text-xs font-semibold text-blue-700 dark:text-blue-300 self-start sm:self-auto">
            ...
            <div x-data="{ userId: null }" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                @foreach ($groups as $index => $group)
                    @php
                        ...
                        if(empty($initials)) { $initials = 'GP'; }

                        $colors = [
                            ['bg' => 'bg-emerald-600 shadow-emerald-600/30', 'badge' => 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400'],
                            ['bg' => 'bg-blue-600 shadow-blue-600/30', 'badge' => 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400'],
                            ['bg' => 'bg-amber-600 shadow-amber-600/30', 'badge' => 'bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400'],
                            ['bg' => 'bg-violet-600 shadow-violet-600/30', 'badge' => 'bg-violet-50 dark:bg-violet-900/30 text-violet-600 dark:text-violet-400'],
                            ['bg' => 'bg-rose-600 shadow-rose-600/30', 'badge' => 'bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400']
                        ];
                        $selectedColor = $colors[$index % count($colors)];
                    @endphp

                    <div wire:key="{{ $group['id'] }}" class="relative w-full bg-white dark:bg-gray-900 border border-slate-200/90 dark:border-gray-700 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between p-5 group/card">

                        <!-- Top Colorful Avatar Block -->
                        <div class="flex flex-col items-center pt-2 pb-4">
                            <div class="w-16 h-16 rounded-2xl {{ $selectedColor['bg'] }} text-white font-bold text-lg flex items-center justify-center shadow-lg mb-3 tracking-wider group-hover/card:scale-105 transition-transform duration-300">
                                {{ $initials }}
                            </div>

                            <!-- Small Category Tag Pill -->
                            <span class="px-3 py-0.5 {{ $selectedColor['badge'] }} text-[10px] font-bold tracking-wider uppercase rounded-full mb-2">
                                GROUP
                            </span>
                            ...
                            @if ($editingGroupId === $group['id'])
                                ...
                                <div class="flex gap-2 w-full">
                                    ...
                                    >
                                        <svg wire:loading wire:target="saveGroupName" class="w-3.5 h-3.5 animate-spin text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            ...
                            @else
                                <h3 class="text-base sm:text-lg font-bold text-gray-900 dark:text-white text-center truncate w-full px-2" title="{{ $group['label'] }}">
                                    {{ $group['label'] }}
                                </h3>
                            @endif
                        </div>
                        ...
                        <div class="mt-4 pt-3 border-t border-slate-100 dark:border-gray-800 grid grid-cols-2 gap-2">
                            ...
                            >
                                <svg class="w-3.5 h-3.5 mr-1.5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M21.1213 2.70705C19.9497 1.53548 18.0503 1.53547 16.8787 2.70705L15.1989 4.38685L7.29289 12.2928C7.16473 12.421 7.07382 12.5816 7.02986 12.7574L6.02986 16.7574C5.94466 17.0982 6.04451 17.4587 6.29289 17.707C6.54127 17.9554 6.90176 18.0553 7.24254 17.9701L11.2425 16.9701C11.4184 16.9261 11.5789 16.8352 11.7071 16.707L19.5556 8.85857L21.2929 7.12126C22.4645 5.94969 22.4645 4.05019 21.2929 2.87862L21.1213 2.70705Z" fill="currentColor"></path>
                                </svg>
                                Edit
                            </button>
