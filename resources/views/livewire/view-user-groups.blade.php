            ...
            <div class="w-full mb-6">
                ...
                <div class="relative" x-data="{ loadingUserId: null }">
                    ...
                    <button id="dropdownGroupUsersButton" data-dropdown-toggle="dropdownGroupUsers"
                        ...
                        Select User
                        <svg class="w-2.5 h-2.5 ml-3" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 10 6">
                                ...
                                @foreach ($availableUsers as $user)
                                    <li class="flex justify-between items-center py-2 px-2 rounded hover:bg-gray-100">
                                        <div class="flex items-center">
                                            <div
                                                class="w-7 h-7 ml-3 flex items-center justify-center text-white rounded-full bg-blue-600">
                                            ...
                                            {{-- <label
                                                class="ml-2 text-sm font-medium text-gray-900">{{ $user['name'] }}</label> --}}
                                        </div>
                                        <button type="button" @click="loadingUserId = {{ $user['id'] }}"
                                            ...
                                            <svg x-show="loadingUserId === {{ $user['id'] }}"
                                                xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"
                                                preserveAspectRatio="xMidYMid" width="20" height="20">
                                                <circle stroke-dasharray="164.93361431346415 56.97787143782138" r="35"
                                                    stroke-width="10" stroke="#ffffff" fill="none" cy="50"
                                                    cx="50">
                    ...
                    <ul class="max-h-60 overflow-y-auto divide-y divide-gray-200">
                        @foreach ($groupUsers as $user)
                            <li class="flex justify-between px-3 py-2">
                                ...
                                <button @click="loadingUserId = {{ $user['id'] }}"
                                    x-bind:disabled="loadingUserId === {{ $user['id'] }}"
